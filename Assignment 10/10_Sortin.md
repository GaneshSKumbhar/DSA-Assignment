# Employee Sorting using Selection Sort and Merge Sort (Performance Analysis)

## Theory
This program compares two classic sorting algorithms:
1. **Selection Sort**  
   - A simple comparison-based algorithm.  
   - Repeatedly selects the maximum (or minimum) element and places it in the correct position.  
   - Time Complexity: O(n²)  
...
The sorting criterion is based on the **average of height and weight** of each employee, in **descending order**.
...
### Selection Sort
1. For each position `i` in the array, find the employee with the **largest average** among remaining elements.  
...
3. Merge the two sorted halves while maintaining descending order of averages.  
...
Employees sorted by Selection Sort (descending avg):
Name    Height  Weight  Average
Charlie 190     90      140
Alice   180     75      127.5
Diana   170     85      127.5
Bob     160     65      112.5
Ethan   155     60      107.5
Selection Sort Time: 15 microseconds

Employees sorted by Merge Sort (descending avg):
Name    Height  Weight  Average
Charlie 190     90      140
Alice   180     75      127.5
Diana   170     85      127.5
